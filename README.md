...
1. Read the Crash Course (~15 minutes) and Common Misconceptions. If unfamiliar, deep-dive with AI and fix gaps.
2. Do the Quick Checks.
3. Work the Problems in order: L1 (Recognition) through L7 (Principal Reasoning), with debugging problems interleaved by difficulty and tagged by type rather than segregated (so you don't know a problem is "the broken one" before reading it).
...
│   └── grand-projects/            GP-1 : Monster project
│
└── assessments/
...
Start solving - see [`PROGRESS.md`](PROGRESS.md) for where to begin and how completion is tracked. As you work a chapter or project, give it its own `CMakeLists.txt` and register it as a subdirectory in the root [`CMakeLists.txt`](CMakeLists.txt), following the `sandbox/` target as a template.
